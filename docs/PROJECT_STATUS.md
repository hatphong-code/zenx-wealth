...
- Savings Escalator + Coast FI (`/savings-escalator`, premium) — input form (7 fields: startMonthly, monthlyGrowthPct, monthlyExpense, fiMultiple 25–31×, currentAge, retirementAge, annualRatePct). Results: 4 stat cards, LineChart (continue-vs-coast lines + dashed FI target), yearly table with toggle, conclusion + disclaimer. Part C: savings account schedule (`users/{userId}/savingsSchedule`), 7-day maturity banner, `savingsScheduleReminder` notif pref. Engine: `buildGrowingContributionSeries`, `calculateFITarget`, `findCoastPoint` in `financialCalculations.js` with Decimal precision + 6 unit tests. UX polish: FI multiple uses custom `FiMultipleSelect` dropdown (token-based, both themes); all result money values use `fmt(value, currency)` respecting user compact/full config. Bug fix: conclusion text passes interpolation params directly to `t()`. Smart defaults seeded from Firestore profile on mount: `currentAge` (DOB/ageRange/fallback 30), `monthlyExpense` (profile.monthlyEssentialExpense), `retirementAge` (`max(currentAge+15, 60)` — 15-year minimum runway, 60 floor per Vietnam labor law). Execution Plan feature: `SavingsEscalatorPlan.jsx` (`/savings-escalator/plan/:planId`), monthly accordion check-in + yearly summary table, scenario picker at Coast month. Multi-plan behavioral gate: up to 3 active plans in parallel; 4th plan gated by avg consistency ≥ 80% + min 1 plan ≥ 12 months; blocked if existing pending plan or any active plan <6 months. Pending plan status: auto-activated when conditions met (checked on page load). Firestore: `users/{uid}/savingsPlans/{planId}` + `checkins/{YYYY-MM}` subcollection. **Plan Detail v2 (2026-06-29):** Smart savings book form — month selector (1..coastMonth, default = current month), smart defaults (label auto-set, amount from series, openDate = 1st of month), `bankName` + `interestRate` fields, auto-checkin on save; schedule entries now store `planMonthIdx/bankName/interestRate`. Auto-label format: `{planName} — T{NN} · {bankName} · {MM/YYYY}` (zero-padded, live update on bank name input). ComparisonChart: confidence band (coast=floor, continue=ceiling) + 3-scenario toggle pills + 1 actual line; chart positioned between progress card and tab bar. Month labels locale-aware (`vi-VN`/`en-US` via `toLocaleDateString`). Interest rate badge i18n (`%/năm`/`%/year`). Tab "Kế hoạch" (3rd tab): projection table + 3-scenario conclusion for reviewing the original plan without leaving plan detail page. `scheduleEntries` lifted to main page and shared between chart and `BankScheduleSection`.
...
users/{userId}/savingsSchedule/{scheduleId} ← savings account maturity records (planId, planMonthIdx, bankName, interestRate, amount, openDate, maturityDate)
users/{userId}/savingsPlans/{planId}         ← Coast FI execution plans
users/{userId}/savingsPlans/{planId}/checkins/{YYYY-MM} ← monthly check-in records
