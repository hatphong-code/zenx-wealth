...
- **i18n** — `src/i18n/` · hook `useI18n()` · service-level `getTranslation()` · VI default / EN ready

---

## Folder Architecture

> ⚠️ **TRẠNG THÁI HIỆN TẠI — Đọc kỹ trước khi edit bất kỳ file nào**

### ACTIVE — Codebase đang chạy

`src/main.jsx` → `src/App.jsx` → tất cả import từ `src/` flat (một tầng).
**Chỉ chỉnh sửa các file trong cấu trúc này.**
...
src/
  ...
  auth/                 ← AuthProvider, useAuth, GoogleAuth, EmailAuth
  components/
    ui/                 ← Button, Card, Input, Toast (base components)
    *.jsx               ← Feature components: AppShell, TransactionRow, StatTile...
  data/                 ← accessControl.js, latteOnboarding.js, budgetTemplates.js
  hooks/                ← useTheme, useNumberFormat, useFeatureAccess, data hooks
  i18n/
    dictionaries/       ← vi.js, en.js
    *.jsx / *.js        ← I18nProvider, useI18n, getTranslation
  pages/                ← Page components: Dashboard, Profile, OnboardingFlow...
  services/             ← Firestore logic: transactionService, userService...
  utils/                ← formatters.js, validators...
```

### INACTIVE — Migration target (chưa kết nối)

`src/core/` và `src/web/` tồn tại trong filesystem nhưng **KHÔNG được import bởi `src/main.jsx`**.
Đây là cấu trúc đích cho RN-Ready migration, hiện chưa active.
**Không edit các file trong `src/core/` hay `src/web/` — thay đổi sẽ không có hiệu lực trên app.**

```
src/
  core/     ← INACTIVE — mirror của src/ cho migration RN sau này
  web/      ← INACTIVE — mirror của src/ cho migration RN sau này
```

### Kế hoạch migration (tương lai)

Khi migration hoàn tất, `src/main.jsx` sẽ được đổi sang import `src/web/App.jsx` và toàn bộ code sẽ dùng `src/core/` + `src/web/`. Đến lúc đó CLAUDE.md sẽ được cập nhật.

**Nguyên tắc ranh giới (áp dụng sau khi migration hoàn tất):**
- ✅ `src/core/` import từ `src/core/`
- ✅ `src/web/` import từ `src/core/`
- ❌ `src/core/` KHÔNG import từ `src/web/` hay React Router, Tailwind, DOM APIs
...
### Where to Put Code (Active paths)

| Code type | Location (ACTIVE) |
|-----------|------------------|
| **Services** (Firestore, transactions, cache) | `src/services/` |
| **Data hooks** (useTransactionsData, useAssetsData) | `src/hooks/` |
| **Custom hooks** (useTheme, useI18n, useAuth) | `src/hooks/` hoặc `src/auth/` |
| **Utils** (formatters, validators, calculations) | `src/utils/` |
| **i18n dictionaries** | `src/i18n/dictionaries/` |
| **Auth providers** (GoogleAuth, EmailAuth) | `src/auth/` |
| **Static data** (accessControl, latteOnboarding) | `src/data/` |
| **UI base components** (Button, Card, Input) | `src/components/ui/` |
| **Feature components** (TransactionRow, StatTile) | `src/components/` |
| **Pages** (Dashboard, Transactions, Settings) | `src/pages/` |
...
// ✅ ĐÚNG — qua service (import path từ src/pages/ → src/services/)
...
- `src/i18n/useI18n.js` — Hook truy cập context (React components only)
- `src/i18n/getTranslation.js` — Service-level i18n (no React dependency)
