...
- **Zustand** — global state (không dùng — dùng React Context + custom hooks)
- **i18n** — `src/core/i18n/` · hook `useI18n()` · service-level `getTranslation()` · VI default / EN ready

---

## Folder Architecture (v3.0 — RN-Ready)

Codebase được tách thành **core** (platform-agnostic) và **web** (web-specific) để chuẩn bị cho React Native:

```
src/
  main.jsx              ← Entry point (Vite) — import từ core + web
  index.css             ← Tailwind + design tokens

  core/                 ← PORTABLE — dùng chung web + RN
    /auth               Services: Google/Apple/Email auth
    /data               Access control, feature catalog, latteOnboarding
    /hooks              Data hooks (useTransactionsData, etc.), custom hooks
    /i18n               Dictionaries (vi.js, en.js), getTranslation() cho services
    /services           Firestore logic: transactionService, userService, etc.
    /utils              formatters, validators, calculations (zero dependencies)

  web/                  ← WEB-ONLY — React UI layer
    /components/ui      Base components: Button, Card, Input, Toast
    /components/        Feature components: AppShell, AppNav, GlobalSearch...
    /hooks/             Web-specific hooks: usePushNotification, useQueueProcessor...
    /pages/             Page-level components (mounted by React Router)
    /App.jsx            Route definitions + feature gating
```

**Nguyên tắc ranh giới:**
- ✅ `src/core/` có thể import từ `src/core/`
- ✅ `src/web/` có thể import từ `src/core/`
- ❌ `src/core/` **KHÔNG ĐƯỢC** import từ `src/web/`
- ❌ `src/core/` **KHÔNG ĐƯỢC** import: react-router-dom, Tailwind, DOM APIs, window.*

**Tại sao?** Khi build React Native app sau này, sẽ copy `src/core/` nguyên vẹn — không cần thay đổi, chỉ tạo RN-specific `components/` + `pages/`.
...
Route definitions + feature gating: `src/web/App.jsx`
Shell: `src/web/components/AppShell.jsx` — không tạo thêm nav wrapper trong pages.

---

## Code Conventions

### Where to Put Code (Core vs Web)

| Code type | Location | Shared? |
|-----------|----------|---------|
| **Services** (Firestore, transactions, cache) | `src/core/services/` | Web + RN |
| **Data hooks** (useTransactionsData, useAssetsData) | `src/core/hooks/` | Web + RN |
| **Custom hooks** (useTheme, useI18n, useAuth) | `src/core/hooks/` | Web + RN |
| **Utils** (formatters, validators, calculations) | `src/core/utils/` | Web + RN |
| **i18n dictionaries** | `src/core/i18n/dictionaries/` | Web + RN |
| **Auth providers** (GoogleAuth, EmailAuth) | `src/core/auth/` | Web + RN |
| **Static data** (accessControl, latteOnboarding) | `src/core/data/` | Web + RN |
| **UI base components** (Button, Card, Input) | `src/web/components/ui/` | Web only |
| **Feature components** (TransactionRow, StatTile) | `src/web/components/` | Web only |
| **Pages** (Dashboard, Transactions, Settings) | `src/web/pages/` | Web only |

**Rule of thumb:** If it doesn't depend on React Router, Tailwind, or DOM APIs → goes in `src/core/`.

### Services — Firestore Operations (Critical Rule)

**Quy tắc tuyệt đối:** Pages KHÔNG ĐƯỢC gọi `firebase/firestore` trực tiếp. Mọi write operation phải qua service layer.

**Pattern:**
```js
// ❌ SAI — pages không import Firebase
import { addDoc, collection, setDoc } from 'firebase/firestore/lite';
function handleSave() {
  const ref = await addDoc(collection(db, 'users/...')); // ❌
}

// ✅ ĐÚNG — qua service (import path từ src/web/pages/ → src/core/services/)
import { createTransaction } from '../../core/services/transactionService';
function handleSave() {
  await createTransaction(userId, { amount, category }); // ✅
}
```

**Service exports:**
- `get*()` — fetch + cache
- `getCached*()` — return cached value only
- `create*()` / `update*()` / `delete*()` — Firestore writes
- `invalidate*Cache()` — clear cached data (dùng qua `cacheCoordinator`)

**Cache invalidation:**
```js
// ✅ Sau khi write, LUÔN gọi cacheCoordinator
import { invalidateAfterTransactionWrite } from '../../core/services/cacheCoordinator';
await createTransaction(userId, data);
invalidateAfterTransactionWrite(userId);
```

**Storage Adapter (cho React Native sau này):**

sessionCache dùng `storageAdapter.js` — không hardcode `window.sessionStorage`. Cho phép RN inject AsyncStorage:
```js
// Web (mặc định)
// → dùng window.sessionStorage

// React Native (sau này)
import AsyncStorage from '@react-native-async-storage/async-storage';
import { setStorageAdapter } from '@core/services/storageAdapter';
setStorageAdapter(AsyncStorage); // RN app startup
```

Tất cả cache operations qua adapter — zero changes cần thiết khi migrate RN.
...
**Files:**
- `src/core/i18n/I18nProvider.jsx` — Provider bọc toàn bộ app
- `src/core/i18n/useI18n.js` — Hook truy cập context (React components only)
- `src/core/i18n/getTranslation.js` — Service-level i18n (no React dependency)
- `src/core/i18n/dictionaries/vi.js` — Tiếng Việt (mặc định)
- `src/core/i18n/dictionaries/en.js` — Tiếng Anh (đầy đủ)

Locale được lưu trong `localStorage` với key `'zx-locale'`. Supported: `['vi', 'en']`.

**Trong React components — dùng Hook:**
```jsx
const { t, locale, setLocale } = useI18n();
```

**Trong Services — dùng getTranslation():**
```js
import { getTranslation, setCurrentLocale } from '../i18n/getTranslation';

function myService(locale = 'vi') {
  setCurrentLocale(locale);
  const title = getTranslation('myKey.title', { param: value });
}
```
...
**Data hooks** (dùng cho pages để fetch data):
```js
import { useTransactionsData } from '../../core/hooks/useTransactionsData';

export function MyPage() {
  const { data, loading, error } = useTransactionsData(userId);
  // Tự động cache, refetch, state management
}
```

**Pattern:** Tất cả 10 data hooks (useAssetsData, useDebtData, useTransactionsData, etc.) được tạo từ `createDataHook()` factory — không code boilerplate.

**Custom hooks** (UI state, theme, auth, i18n):
- `useTheme()` → lấy theme + toggle theme
- `useI18n()` → lấy t(), locale, setLocale
- `useAuth()` → lấy user, auth state, sign in/out
- `useFocusTrap()` — trap keyboard focus trong modal/dialog
