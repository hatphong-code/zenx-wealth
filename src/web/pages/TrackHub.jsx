...
import { createEmergencyFundRecord, setEmergencyFundCache } from '../../core/services/emergencyFundService';
import { fmtShort, formatDate, formatMoney } from '../../core/utils/formatters';
...
function ConvertSheet({ latteTotal, currency, emergencyData, userId, onClose, onSuccess }) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const result = await createEmergencyFundRecord(userId, record);
      const nextData = {
        ...emergencyData,
        records: [{ id: result.id, ...record }, ...records],
      };
      setEmergencyFundCache(userId, nextData);
      invalidateAfterEmergencyFundWrite(userId);
